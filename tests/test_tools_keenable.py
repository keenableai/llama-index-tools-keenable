...
def test_search_text_prefers_snippet_over_empty_description(monkeypatch):
    """The API returns both fields, and `description` is usually the empty one.

    Reading `description` first left every Document's text as its bare title, which
    is what shipped in 0.1.2 and earlier — near useless for retrieval.
    """
    body = {
        "results": [
            {
    ...
    assert docs[0].text == "the page text"
    # The raw result still reaches metadata untouched, both fields included.
    assert docs[0].metadata["snippet"] == "the page text"
    assert docs[0].metadata["description"] == ""


def test_search_text_falls_back_to_description_then_title(monkeypatch):
    body = {"results": [{"title": "T", "url": "https://e.com", "description": "d"}]}
    _patch(monkeypatch, _FakeResponse(json_body=body))
    assert KeenableToolSpec().search("q")[0].text == "d"
...
def test_search_bad_payload_redacts_key(monkeypatch):
    ...
    with pytest.raises(KeenableError) as exc:
        KeenableToolSpec(api_key=key).search("q")
    assert key not in str(exc.value)
    assert "***" in str(exc.value)
